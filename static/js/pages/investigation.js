...
UGPages.investigation = (() => {
  ...
  function renderStandardizedSchema(inc, isLive) {
    const schemaEl = document.getElementById("invAlertSchema");
    const ledgerEl = document.getElementById("invLedgerKv");
    if (!schemaEl || !ledgerEl) return;

    // Standardized fields are written by IncidentManager for REAL (live)
    // incidents only - demo/simulated incidents come from the client-side
    // demo engine, which does not carry these SIH26145 schema fields.
    if (!isLive || inc.is_phone_device || inc.threat_class === undefined) {
      schemaEl.innerHTML = `<div><span>Data source</span><span class="src-badge src-SIMULATED">🟣 SIMULATED / DEMO</span></div>`;
      schemaEl.innerHTML += `<div style="grid-column:1/-1; font-size:11px; color:var(--muted);">Standardized alert schema fields (threat class, confidence, source-IP authenticity, evidence hashes) are populated for REAL live incidents only.</div>`;
      ledgerEl.innerHTML = `<div style="grid-column:1/-1; font-size:11px; color:var(--muted);">Evidence Ledger records real live incidents only - not demo/simulated ones.</div>`;
      return;
    }

    const auth = inc.source_ip_authenticity || {};
    schemaEl.innerHTML = `
      <div><span>Threat Class</span><span>${inc.threat_class || "-"}</span></div>
      <div><span>Confidence</span><span>${inc.confidence != null ? (inc.confidence * 100).toFixed(1) + "%" : "-"}</span></div>
      <div><span>Flow ID</span><span>${inc.flow_id || inc.id}</span></div>
      <div><span>Protocol</span><span>${inc.protocol || "-"}</span></div>
      <div><span>Destination IP</span><span>${inc.destination_ip || "-"}</span></div>
      <div><span>Source-IP Authenticity</span><span class="badge ${auth.assessment === "Potentially Spoofed / Automated Source" ? "b-HIGH" : "b-LOW"}">${auth.assessment || "-"}</span></div>
    `;
    if (auth.indicators && auth.indicators.length) {
      schemaEl.innerHTML += `<div style="grid-column:1/-1;"><span style="color:var(--muted); font-size:10.5px;">Authenticity indicators</span><ul class="evidence-list">${auth.indicators.map(i => `<li>${i}</li>`).join("")}</ul></div>`;
    }

    ledgerEl.innerHTML = `
      <div><span>Integrity Status</span><span class="badge ${inc.integrity_status === "VERIFIED" ? "b-VERIFIED" : "b-TAMPERED"}">${inc.integrity_status || "-"}</span></div>
      <div><span>Evidence Record</span><span>${inc.evidence_record_id || "-"}</span></div>
      <div><span>Evidence Hash</span><span>${truncHash(inc.evidence_hash)}</span></div>
      <div><span>Previous Record Hash</span><span>${truncHash(inc.previous_record_hash)}</span></div>
      <div><span>Current Record Hash</span><span>${truncHash(inc.current_record_hash)}</span></div>
    `;
  }
  ...
  function render(state) {
    ...
    renderStandardizedSchema(inc, isLive);
